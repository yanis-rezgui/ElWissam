...
const DeleteCommunePop = ({ commune }: { commune: Commune }) => {

    const { setShowDeletePop, deleteCommune, loadingDeleteCommune } = useAdminCommuneContext();
        ...
        <div onClick={() => setShowDeletePop(false)} className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 p-4">
            ...
            >
                <p className="text-center text-[18px] mt-5 text-[#0F172A]">
                    Etes vous sûr de vouloir supprimer la commune : <strong>{commune.name}</strong> ?
                </p>
                ...
                "
                    >
                        {loadingDeleteCommune ? "Suppression..." : "Oui, Supprimer"}
                    </button>
...
export default memo(DeleteCommunePop);